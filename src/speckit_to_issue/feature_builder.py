...
class FeatureIssueBuilder:
    ...
    the entire feature systematically. Includes full spec documents
    for maximum context.
    """

    def __init__(self, spec_name: str, copilot_mode: bool = False, feature_title: str = ""):
        """Initialize the builder.
        
        Args:
            spec_name: Name of the spec (e.g., "003-help-faq-page")
            copilot_mode: If True, include Copilot-specific instructions
            feature_title: Explicit feature title (from tasks.md header)
        """
    ...
    def build(
        ...
        # Title from spec name or explicit title
        ...
        # Add verbose spec documents in collapsible sections
        verbose_section = self._build_verbose_spec_sections(spec_context)
        if verbose_section:
            sections.append(verbose_section)
    ...
    def _format_title(self) -> str:
        """Format the issue title from spec name or explicit title.
        
        Uses feature_title if available, otherwise converts
        "003-help-faq-page" to "Feature: Help FAQ Page".
        """
        if self.feature_title:
            return f"Feature: {self.feature_title}"
...
Use the architecture section above for guidance on structure and key decisions."""
    ...
    def _build_verbose_spec_sections(self, spec_context: Optional[SpecContext]) -> str:
        """Build collapsible sections containing full spec documents.
        
        Each document is wrapped in a ``<details>`` tag for readability.
        
        Args:
            spec_context: Extracted spec context with raw file contents
            
        Returns:
            Formatted collapsible sections, or empty string if no content
        """
        if not spec_context:
            return ""
        
        items: list[tuple[str, str, str]] = [
            ("📋 Full Specification (spec.md)", spec_context.raw_spec, "spec"),
            ("📐 Implementation Plan (plan.md)", spec_context.raw_plan, "plan"),
            ("📝 Task Breakdown (tasks.md)", spec_context.raw_tasks, "tasks"),
            ("🔬 Research (research.md)", spec_context.raw_research, "research"),
            ("📊 Data Model (data-model.md)", spec_context.raw_data_model, "data-model"),
            ("🚀 Quickstart (quickstart.md)", spec_context.raw_quickstart, "quickstart"),
            ("📜 Project Constitution", spec_context.raw_constitution, "constitution"),
        ]
        
        blocks: list[str] = []
        for label, content, _key in items:
            if not content:
                continue
            blocks.append(
...
def build_feature_issue(
    ...
    Returns:
        Issue object ready for creation
    """
    builder = FeatureIssueBuilder(spec_name, copilot_mode=copilot_mode, feature_title=feature_title)
